...
describe("demo-site integration architecture", () => {
  ...
  it("resolves the Project Hero and Shared fallbacks through the runtime registry factory", () => {
    ...
    expect(resolveSection(registry, { type: "hero/HeroFullscreen" })).toBe(
      "project-hero",
    );
    expect(resolveSection(registry, { type: "header/Header" })).toBe(
      "shared-header",
    );
    expect(resolveSection(registry, { type: "features/FeatureGrid" })).toBe(
      "shared-features",
    );
    expect(resolveSection(registry, { type: "cta/CTA" })).toBe("shared-cta");
    expect(() => resolveSection(registry, { type: "missing/Section" })).toThrow(
      'Unknown Web Factory section "missing/Section".',
    );
  });
  ...
  it("keeps the runtime CSS imports in Shared → Template → Theme → Project order", () => {
    ...
    expect(
      readFileSync(resolve(root, "src/styles/project.css"), "utf8"),
  ...
  it("switches one selected theme while preserving Project CSS last", () => {
    ...
    const luxuryStyles = resolveStyleComposition(
      { ...project, theme: "luxury" },
      catalog,
    );
