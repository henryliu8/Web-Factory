...
import { renderStyleComposition, resolveStyleComposition } from './styleRuntime';
...
describe('Web Factory style runtime', () => {
  it('resolves the default theme in runtime cascade order', () => {
    expect(resolveStyleComposition({ template: 'stardrive', theme: 'default' }, catalog)).toEqual([
      '@webfactory/tokens',
  ...
  it('resolves only the luxury theme when selected', () => {
    const styles = resolveStyleComposition({ template: 'stardrive', theme: 'luxury' }, catalog);
  ...
  it('rejects an invalid theme with the available choices', () => {
    expect(() => resolveStyleComposition({ template: 'stardrive', theme: 'corporate-x' }, catalog))
      .toThrow('Unknown Web Factory theme: corporate-x\n\nAvailable themes:\n- default\n- luxury');
  });

  it('rejects an invalid template with the available choices', () => {
    expect(() => resolveStyleComposition({ template: 'missing', theme: 'default' }, catalog))
      .toThrow('Unknown Web Factory template: missing\n\nAvailable templates:\n- stardrive');
  });
  ...
  it('renders the concrete selected entries into the stylesheet Astro loads', () => {
    ...
    expect(rendered).toContain("@import '@webfactory/template-stardrive/styles'");
