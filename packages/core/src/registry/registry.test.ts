...
function layers(
  include: Partial<Record<"template" | "theme" | "project", boolean>> = {},
): RegistryLayer<string>[] {
  return [
    {
      name: "shared",
      entries: { [key]: "shared", "header/Header": "shared-header" },
    },
    ...(include.template
      ? [{ name: "template" as const, entries: { [key]: "template" } }]
      : []),
    ...(include.theme
      ? [{ name: "theme" as const, entries: { [key]: "theme" } }]
      : []),
    ...(include.project
      ? [{ name: "project" as const, entries: { [key]: "project" } }]
      : []),
  ];
}
...
describe("layered registry", () => {
  ...
  it("lets Template override Shared", () => {
    expect(resolveEntry(createRegistry(layers({ template: true })), key)).toBe(
      "template",
    );
  });
  ...
  it("lets Theme override Template", () => {
    expect(
      resolveEntry(
        createRegistry(layers({ template: true, theme: true })),
        key,
      ),
    ).toBe("theme");
  });
  ...
  it("directly proves Project > Theme > Template > Shared", () => {
    expect(
      resolveEntry(
        createRegistry(layers({ template: true, theme: true, project: true })),
        key,
      ),
    ).toBe("project");
  });
  ...
  it("does not mutate registry inputs", () => {
    const input = [
      ...layers({ template: true, theme: true, project: true }),
    ].reverse();
  ...
  it("uses full logical paths rather than basenames", () => {
    const registry = createRegistry([
      {
        name: "shared",
        entries: { "hero/Card": "hero", "features/Card": "feature" },
      },
    ]);
