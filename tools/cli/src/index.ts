...
program
  ...
  .action(async (project, options) =>
    run(() => createProject(project, options)),
  );
...
const list = program
  .command("list")
  .description("List available Web Factory resources.");
list
  .command("templates")
  .action(() => listTemplates().forEach((item) => console.log(item)));
list
  .command("themes")
  .action(() => listThemes().forEach((item) => console.log(item)));
