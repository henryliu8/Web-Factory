...
export async function runProjectCommand(
  ...
  if (!existsSync(directory))
    throw new Error(
      `Web Factory could not find project "${project}" at ${directory}.`,
    );
  ...
  await execa(
    ...
    [
      "exec",
      "astro",
      command === "dev" ? "dev" : command === "check" ? "check" : "build",
    ],
...
async function validateProject(
  ...
  if (!existsSync(configPath))
    throw new Error(`Web Factory project is missing ${configPath}.`);
  const module = (await import(
    `${pathToFileURL(configPath).href}?wf=${Date.now()}`
  )) as { default?: unknown };
...
export async function syncProject(
  ...
  throw new Error(
    `Web Factory sync is not implemented yet for "${project}". No project files were changed.`,
  );
}
