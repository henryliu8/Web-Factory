...
export function listTemplates(start = process.cwd()): string[] {
  return listPackages(resolve(repositoryRoot(start), 'templates'), '@webfactory/template-');
}

export function listThemes(start = process.cwd()): string[] {
  return listPackages(resolve(repositoryRoot(start), 'themes'), '@webfactory/theme-');
}
