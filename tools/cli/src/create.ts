...
export async function createProject(
  ...
  if (existsSync(destination)) {
    throw new Error(
      `Project "${slug}" already exists at ${destination}. Choose another name.`,
    );
  }
  ...
  const pages = normalizePages(
    options.pages ? parsePages(options.pages) : await selectPages(),
  );
...
function validateChoice(label: string, value: string, choices: string[]): void {
  if (!choices.includes(value)) {
    throw new Error(
      `Web Factory could not find ${label} "${value}".\n\nAvailable ${label}s:\n${choices.map((item) => `- ${item}`).join("\n")}`,
    );
  }
}
...
export function parsePages(value: string): string[] {
  const pages = value
    ...
    .filter(Boolean);
  if (!pages.length)
    throw new Error("The --pages option must contain at least one page.");
  return pages;
}
...
async function selectTemplate(cwd: string): Promise<string> {
  const result = await prompts.select({
    ...
    options: listTemplates(cwd).map((value) => ({ value, label: value })),
  });
...
async function selectTheme(cwd: string): Promise<string> {
  const result = await prompts.select({
    ...
    options: listThemes(cwd).map((value) => ({ value, label: value })),
  });
  if (prompts.isCancel(result)) {
    prompts.cancel("Project creation cancelled.");
    throw new Error("Project creation cancelled.");
...
async function selectPages(): Promise<string[]> {
  const result = await prompts.multiselect({
    ...
    options: defaultPages.map((value) => ({
      value,
      label: value,
      hint: value === "home" ? "required" : undefined,
    })),
    initialValues: defaultPages,
  });
  if (prompts.isCancel(result)) {
    prompts.cancel("Project creation cancelled.");
    throw new Error("Project creation cancelled.");
...
async function validateGeneratedProject(directory: string): Promise<void> {
  ...
  for (const path of textFiles) {
    if (placeholderPattern.test(await readFile(path, "utf8")))
  ...
  const configFiles = textFiles.filter(
    (path) => basename(path) === "webfactory.config.ts",
  );
  if (configFiles.length !== 1) {
    throw new Error(
      `Generated project must contain exactly one webfactory.config.ts; found ${configFiles.length}.`,
    );
  }
...
export function normalizePageName(value: string): string {
  ...
  if (!input || input === "." || input === ".." || /[\\/]/.test(input)) {
    throw new Error(
      `Invalid page name "${value}". Page names cannot be paths.`,
    );
  }
  const slug = input.toLowerCase().replace(/[\s_]+/g, "-");
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) {
    throw new Error(
      `Invalid page name "${value}". Use letters, numbers, spaces, underscores, or hyphens.`,
    );
  }
