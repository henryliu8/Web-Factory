...
describe("createProject", () => {
  it("creates scaffold and project files without copying shared packages", async () => {
    ...
    expect(
      JSON.parse(await readFile(join(path, "package.json"), "utf8")).name,
    ).toBe("@webfactory/app-forest-flooring");
    expect(
      await readFile(join(path, "webfactory.config.ts"), "utf8"),
    ).toContain("theme: 'luxury'");
    expect(
      await readFile(join(path, "src/pages/about.astro"), "utf8"),
    ).toContain(
      "@webfactory/template-stardrive/page-templates/StandardPageTemplate",
    );
  ...
  it("fails cleanly when a scaffold placeholder is unresolved", async () => {
    const root = await createFixture();
    await writeFile(
      join(root, "templates/stardrive/scaffold/broken.txt"),
      "{{unknownValue}}",
    );
    ...
    expect(
      (await readdir(join(root, "apps"))).some((name) =>
        name.startsWith(".wf-broken-site-"),
      ),
    ).toBe(false);
  });
  ...
  it("rejects unknown templates and themes with available choices", async () => {
    ...
    await expect(
      createProject(
        "unknown-template",
        { template: "missing", theme: "default", pages: "home" },
        root,
      ),
    ...
    await expect(
      createProject(
        "unknown-theme",
        { template: "stardrive", theme: "missing", pages: "home" },
        root,
      ),
...
describe("real Stardrive scaffold generation", () => {
  it("processes the actual scaffold into a structurally valid project", async () => {
    ...
    const text = (
      await Promise.all(
        generatedFiles
          .filter((file) =>
            [".astro", ".css", ".json", ".md", ".mjs", ".ts", ".txt"].includes(
              extname(file),
            ),
          )
    ...
    const homePage = await readFile(
      join(path, "src/pages/index.astro"),
      "utf8",
    );
    const aboutPage = await readFile(
      join(path, "src/pages/about-us.astro"),
      "utf8",
    );

    expect(basename(path)).toBe("my-new-website");
    expect(manifest.name).toBe("@webfactory/app-my-new-website");
    expect(manifest.dependencies["@webfactory/template-stardrive"]).toBe(
      "workspace:*",
    );
    expect(manifest.dependencies["@webfactory/theme-luxury"]).toBe(
      "workspace:*",
    );
    ...
    expect(
      generatedFiles.filter(
        (file) => basename(file) === "webfactory.config.ts",
      ),
    ).toHaveLength(1);
    ...
    expect(await readFile(join(path, "src/config/site.ts"), "utf8")).toContain(
      "name: 'My New Website'",
    );
    expect(homePage).toContain(
      "@webfactory/core/page-builder/PageBuilder.astro",
    );
    expect(homePage).toContain(
      "@webfactory/template-stardrive/layouts/SiteLayout",
    );
    ...
    const globalStyles = await readFile(
      join(path, "src/styles/global.css"),
      "utf8",
    );
    ...
    expect(
      globalStyles.indexOf("@import 'virtual:webfactory-theme-style'"),
    ).toBeLessThan(globalStyles.indexOf("@import './project.css'"));
    expect(await readFile(join(path, "astro.config.mjs"), "utf8")).toContain(
      "createStyleRuntime",
    );
  });
  ...
  it("selects only the default theme package in a generated default project", async () => {
    ...
    expect(manifest.dependencies["@webfactory/theme-default"]).toBe(
      "workspace:*",
    );
    expect(manifest.dependencies["@webfactory/theme-luxury"]).toBeUndefined();
    expect(
      (await readFile(join(path, "src/styles/global.css"), "utf8")).trimEnd(),
...
describe("safe naming and repository discovery", () => {
  ...
  it.each(["../escape", "/absolute", "nested/project", ".."])(
    "rejects unsafe project name %s",
    (name) => {
      expect(() => normalizeProjectSlug(name)).toThrow(
        "Project names cannot be paths",
      );
    },
  );

  it.each(["../escape", "nested/page", ".."])(
    "rejects unsafe page name %s",
    (name) => {
      expect(() => normalizePageName(name)).toThrow(
        "Page names cannot be paths",
      );
    },
  );
...
async function createFixture(realScaffold = false): Promise<string> {
  ...
  await Promise.all([
    ...
    writeFile(
      join(root, "templates/stardrive/package.json"),
      '{"name":"@webfactory/template-stardrive"}',
    ),
    writeFile(
      join(root, "themes/default/package.json"),
      '{"name":"@webfactory/theme-default"}',
    ),
    writeFile(
      join(root, "themes/luxury/package.json"),
      '{"name":"@webfactory/theme-luxury"}',
    ),
  ]);

  const scaffold = join(root, "templates/stardrive/scaffold");
  if (realScaffold) {
    await cp(join(repositoryRoot(), "templates/stardrive/scaffold"), scaffold, {
      recursive: true,
    });
  } else {
    ...
    await Promise.all([
      writeFile(
        join(scaffold, "package.json.template"),
        '{"name":"@webfactory/app-{{project}}"}',
      ),
      writeFile(
        join(scaffold, "webfactory.config.ts"),
        "template: '{{template}}'; theme: '{{theme}}'; pages: {{pages}};",
      ),
      writeFile(
        join(scaffold, "src/config/site.ts"),
        "name: '{{projectName}}';",
      ),
    ]);
  }
